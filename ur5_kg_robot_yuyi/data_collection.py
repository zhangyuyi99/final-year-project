...
def press(down_vel,up_vel,distance,hold_time):
    ...
    burt.translatel_rel([0, 0, -distance-0.025, 0, 0, 0], acc=acc, vel = down_vel)
    time.sleep(hold_time)
    burt.translatel_rel([0, 0, distance+0.025, 0, 0, 0], acc=acc, vel = up_vel)
    time.sleep(3)
...
data = []
with open('data/soft_80_data_angle_0.txt', 'w') as f:
    ...
    #                     event = device.read(2)
    #                     output = {'control':control_param,'position_l': position_l, 'position_j':position_j, 'midi': event}
    #                     print(output, file=f)
    #                     print(output)
    #                     data.append(output)
    #
    ...
    # for distance in distance_list:
    #     for down_vel in down_vel_list:
    #         if down_vel**2>2*acc*distance+1:
    #             continue
    #         for up_vel in up_vel_list:
    #             for hold_time in hold_time_list:
    #
    ...
    for down_vel in down_vel_list:
        distance = down_vel**2/(2*acc)+0.03
        for up_vel in up_vel_list:
            for hold_time in hold_time_list:
