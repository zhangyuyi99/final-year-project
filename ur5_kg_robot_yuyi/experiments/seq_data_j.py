...
def move_press(note,down_vel,up_vel,distance,hold_time,down_acc,up_acc,press_depth):


    ready_position_l = note.position[:]
    ready_position_l[2]+=distance
    burt.movel(ready_position_l,acc=up_acc,vel = up_vel)
...
distance_list = np.linspace(0,0.15,3)
down_vel_list = np.linspace(0.01,2.01,11)
up_vel_list = np.linspace(0.01,2.01,11)
hold_time_list = np.linspace(0,1.0,3)
down_vel_list = np.linspace(0.42,0.51,2)
up_vel_list = np.linspace(0.01,0.07,15)[::-1]

for i in range(seq_length):
    note = random.choice(list(p.keys.values()))
    # down_vel = random.choice((0.01, 1.00, 0.01)
    # up_vel = random.randrange(0.01, 1.00, 0.01)
    # move_time = random.randrange(0, 3, 0.1)
    # distance = random.randrange(0,0.15,0.01)
    # hold_time = random.randrange(0, 3, 0.1)
    # down_acc = random.randrange(0.1, 3, 0.1)
    # up_acc = random.randrange(0.1, 3, 0.1)
    # press_depth = random.randrange(0.007, 0.015, 0.001)
    down_vel = random.uniform(0.01, 1.00)
    up_vel = random.uniform(0.01, 1.00)
    # move_time = random.uniform(0, 3)
    distance = random.uniform(0.01,0.08)
    hold_time = random.uniform(0, 3)
    down_acc = random.uniform(0.1, 2)
    up_acc = random.uniform(0.1, 2)
    press_depth = random.uniform(0.007, 0.015)

    # move_press(note,move_time,down_vel,up_vel,distance,hold_time,down_acc,up_acc,press_depth)
    move_press(note, down_vel, up_vel, distance, hold_time, down_acc, up_acc, press_depth)
    ...
    control_param = {'note': note.name, 'down_vel': down_vel, 'up_vel': up_vel,
                     'distance': distance,
                     'hold_time': hold_time, 'down_acc': down_acc, 'up_acc': up_acc, 'press_depth': press_depth}
